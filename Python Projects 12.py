    ...
    while (lanjutTidak == True) :
        ...
        if (jenengBuah not in buahDict.keys ()) :
            ...
            try :
                ...
                print ('input error, hanya tersedia (y/n), silahkan masukkan y atau n')
                continue
            except ValueError :
...
def nambahBuah(buahDict) :
    ...
    while True :
        try :
            ...
            print ('\nInputan Invalid Coba Lagi ')
            continue
def menghapusBuah(buahDict) :
    hapusBuah = input('Masukkan nama buah yang ingin di Hapus : ').lower()
    if (hapusBuah in buahDict.keys()) :
        del buahDict[hapusBuah]
        print('Buah ', hapusBuah, 'buah yang ingin di hapus sudah berhasil')

        print('\nData Buah : ')
        for a,b in buah.items() :
                      print ('- ' ,a, '(harga  : ' , b, ')')
    else :
        print('Buah yang di maksud tidak ada di dalam list data buah, coba lagi deh')
...
print('b. Beli buah')
print('c. Hapus buah di dalam data list buah')
print('97. Keluar')
